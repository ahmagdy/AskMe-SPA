...
export class CommService {
    ...
    constructor(private http: HttpClient, private authService: AuthService,
        private router: Router, private ck: CookieService) { }
    ...
    getUserDetails(username: string) {
        ...
        return this.http.get<IUser>('/api/public/user', { params: new HttpParams().set('username', username) })
            .catch(err => {
                return catchError(err, 'Something went wrong', false);
            });
    }
    ...
    changeVisibility(id: number) {
        return this.http.post('/api/messages/visibility', { id })
            .catch(err => {
                return catchError(err, 'Unable to change message', false);
            });
    }
    deleteMessage(id) {
    ...
    getPreviousReply(id) {
        return this.http.get('/api/messages/reply', { params: new HttpParams().set('id', id) })
            .catch(err => {
                return catchError(err, 'Something wrong', false);
            });
    }
...
function catchError(error, message: string, nav: boolean) {
    console.log(error);
